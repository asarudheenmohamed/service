...
class LowStockNotificationController(object):
    ...
    def get_low_stocks(self):
        ...
        THRESHOLD_QTY = 5
        fast_moving_sku = ['CHK_BIRI_CUT', 'CHK_BONELESS', 'CHK_BR_BONELESS',
                           'CHK_CNTRY_UNIT', 'CHK_CNTRY_WOS_UNIT',
                           'CHK_DRM_SKIN_OFF', 'CHK_EGG', 'CHK_LEG_SKIN_OFF',
                           'CHK_LOLLI', 'CHK_MINCE', 'CHK_SELECT',
                           'CHK_WHL_SKIN_OFF',
                           'CHK_WHL_SKIN_ON', 'GT_BIRYANI_CUT', 'GT_BONELESS',
                           'GT_CURRY_CUT', 'GT_FAT_FREE_BONELESS', 'GT_MINCE',
                           'GT_REG_BONELESS', 'GT_REG_CURRY_CUT',
                           'MRT_CHETTINAD_CHICKEN_65',
                           'MRT_MASRAS_CHICKEN_65_BONELESS', 'SF_ANCHOVY',
                           'SF_BABY_SEER', 'SF_BLUE_CRAB', 'SF_BPOMFRET', 'SF_CATLA',
                           'SF_CATLA_SMALL', 'SF_EMPEROR', 'SF_FWPOMFRET', 'SF_IM',
                           'SF_INDIAN_SALMON', 'SF_LADY_FISH', 'SF_MATHI',
                           'SF_PRAWN', 'SF_ROHU', 'SF_ROHU_SMALL', 'SF_SANKARA',
                           'SF_SEABASS', 'SF_SEER', 'SF_SHARK', 'SF_SHELLA',
                           'SF_TREVLLY', 'SF_TUNA']
        ...
        inventory_objs = GraminventoryLatest.objects.prefetch_related(
            'product').filter(product__sku__in=fast_moving_sku,
                              qty__lte=THRESHOLD_QTY).values_list(*columns)
        ...
        def format_df(group):
            ...
            low_stocks[store_id] = group[
                ['product_id', 'product__sku', 'qty']].to_dict('records')
    ...
    def get_stock_details(self, low_stocks):
        ...
        store_objs = CoreStore.objects.filter(
            store_id__in=low_stocks.keys()).values_list('store_id', 'name')

        #  Inorder to get product name filter the object from CatalogProductFlat1
        product_obj = CatalogProductFlat1.objects.prefetch_related(
            'entity').values_list('sku', 'name')
        ...
        for store_id, products in low_stocks.items():
            ...
            product_detail = ["{}: <b>{} - {}(qty)</b>".format(
                index, product_obj.get(sku=product['product__sku'])[1],
                product['qty'])
