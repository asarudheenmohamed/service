...
class OrderController(object):
    ...
    def complete(self):
        ...
        reward and credit.
        """
        invoice_id = self.mage.api.sales_order_invoice.create({'orderIncrementId':self.order.increment_id})
        self.mage.api.sales_order_invoice.capture({'invoiceIncrementId ': invoice_id})
        response_data=self.mage.api.sales_order_shipment.create({'orderIncrementId':self.order.increment_id})
