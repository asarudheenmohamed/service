...
class JuspayOrderSuccessProcessor(JuspayMixin):
    ...
    def action_order_confirm(self):
        ...
        """
        logger.info(
            "WEBHOOK: Payment ReConfirming {}".format(
        ...
        payment.save()
        logger.info(
            "WEBHOOK: Changed the order:{} payment method-juspay".format(
                self.order.increment_id))

        # SMS TRIGGERING
        tasks.send_sms.delay(self.order.increment_id, 'payment_confirmation')
        logger.info(
            "WEBHOOK: Sent payment confirmation message to the order:{} ".format(
                self.order.increment_id))

    def action_order_refund(self):
        logger.info("WEBHOOK: Payment Refunding {} for {}".format(
            self.order.increment_id,
        ...
        logger.info(
            "WEBHOOK: Payment refunded to the order:{} for {}".format(
                self.order.increment_id,
                self.order.grand_total))
        ...
        logger.info(
            "WEBHOOK: Sent payment refund message to the order:{}".format(
                self.order.increment_id))
    ...
    def execute(self):
        ...
        if status in ("pending",
                      "scheduled_order") and payment.method == "juspay":
            return
